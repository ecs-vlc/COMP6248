...
\item The {\bf discriminator}
	...
	y = d(\bm x)
	\end{equation}
	gets a sample $\bm x$ as input and predicts a probability $y \in [0, 1]$ (or logit of a Bernoulli distribution in $\mathbb{R}$) determining if it is real or fake.
\end{itemize}
\end{frame}
%-------------------------------------------------------------%
...
\begin{enumerate}
\item Oscillations without convergence. Contrary to standard loss minimisation,
